...
# Gemini 2.5 defaults the safety threshold to Off for all four harm categories,
# so benign finance text does not need an explicit override in the request.
...
async def classify_ticket(ticket: TicketRequest) -> TicketResponse:
    ...
    try:
        ...
        # and the user turn as `contents`. We retry transient 5xx with exponential
        # backoff; if the model genuinely returns no text, the outer try/except
        # below turns that into the safe fallback response.
        async def _call_once() -> str:
            ...
            if text:
                return text
            # Surface the finish reason / prompt-feedback so we can diagnose
            # any genuine empty-response case in logs.
            try:
